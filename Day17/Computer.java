...
public class Computer {
    ...
    int oPoint;
    int[] sequence;
    ArrayList<Byte> testCommands = new ArrayList<>();

    Stack<Byte> output = new Stack<>();

    public Computer() {
        this(729, 6);
    }

    public Computer(int registerA, int sequenceLength) {
    ...
    public Computer(int registerA, int[] sequence) {
        this.registerA = registerA;
        this.registerB = 0;
        this.registerC = 0;
        oPoint = 0;
    ...
    public boolean testCommands() {
        System.out.println("\nTesting input: " + registerA);

        ArrayList<Byte> commands = testCommands;

        while (oPoint < commands.size() - 1 && output.size() < sequence.length) {
            byte opcode = commands.get(oPoint);
            byte literal = commands.get(oPoint + 1);
            ...
            switch (opcode) {
                ...
                case 5 -> {
                    ...
                    output.push((byte) result);
                    if (result != sequence[output.size() - 1]) {
                        return false;
                    } else if (output.size() == sequence.length) {
                        return true;
                    }
                    oPoint += 2;
                }
        ...
        if (output.size() != sequence.length) {
            return false;
        }
